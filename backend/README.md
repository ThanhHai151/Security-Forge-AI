...
- **Expose the pillars** as API endpoints:
  ...
  - Router — manage provider accounts (API-key + OAuth sign-in) and rotation policy.
...
- Provider credentials (API keys and OAuth tokens) live only in the backend account store;
  the API layer masks them and never returns a raw secret.
