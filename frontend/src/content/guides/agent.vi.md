...
4. Điền **Quy tắc kiểm thử**: chọn harness tác nhân và mức độ quan trọng của tài sản; nhập tham
   chiếu ủy quyền đã ký, khung giờ kiểm thử, mục tiêu loại trừ và chính sách subdomain; sau đó xác
   nhận ủy quyền bằng văn bản. Có thể để thiếu khi chỉ lập kế hoạch, nhưng harness sẽ chặn mọi lưu
   lượng tới mục tiêu.
5. Bấm **Hỏi chuyên gia giám sát**.

## Bước 3 — Đọc kế hoạch điều tra

Mở ngăn **Plan** để xem trạng thái harness, các trường RoE đang chặn, **thứ tự điều tra** đã xếp
hạng, **loại ứng dụng** được nhận diện
(ví dụ "mạng xã hội" hoặc "quản lý dữ liệu nhiều người dùng"), các **skill** được chọn và
phần **Câu hỏi cần trả lời**. Các câu hỏi tạo thành chuỗi bằng chứng: lập bản đồ bề mặt, nhận diện
implementation, chạy cặp kiểm soát an toàn rồi chứng minh tác động tối thiểu. Câu hỏi có điều kiện
cho agent biết khi nào tiếp tục hoặc loại bỏ một nhánh. Bước đầu tiên tự động được đánh dấu
*đang xử lý* (viền vàng trên kỹ thuật đó trong danh mục).

## Bước 4 — Thực thi bằng coding agent của bạn

Nhấn **Sao chép harness** rồi đưa toàn bộ briefing (RoE, gate, giai đoạn, kế hoạch, câu hỏi và
skill) cho agent bên ngoài đã chọn (Claude Code, OpenAI Codex, Cursor hoặc host khác). Yêu cầu agent
trả lời từng câu hỏi bằng log hoặc mã nguồn và bỏ qua điều kiện không đúng. **SecForge tư vấn;
agent thực thi** —
